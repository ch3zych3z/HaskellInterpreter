...
-- a |-| b = HEBinOp a Sub b
-- a |/| b = HEBinOp a Div b
-- a |==| b = HEBinOp a Eqls b
-- zero = HEVal $ HVInt 0
-- one = HEVal $ HVInt 1
-- two = HEVal $ HVInt 2

-- con = HEAbs "x" $ HEAbs "y" $ HEVar "x"
-- undef = HEAbs "x" $ HEVar "undef" |$| HEVar "x"
...
--     HEIf (HEVar "x" |==| one)
--       zero 
--       $ one |+| (HEVar "log2" |$| (HEVar "x" |/| two))
--   )]
-- testLazy a =
--   HELet $ Binds
--     [] $ HEVar "log2" |$| HEVal (HVInt a)
-- testLazy =
--   HELet $ Binds 
--     [
--       Bind "fac" $
...
--           HEIf (HEVar "x" |==| one)
--             one
--             $ HEVar "x" |*| (HEVar "fac" |$| (HEVar "x" |-| one))
--     ]
--     $ HEVar "fac" |$| (HEVar "fac" |$| (one |+| two))
-- testLazy = 
--   HELet "con" con $
--     HELet "undef" undef $
--       (HEVar "con" |$| one) |$| HEVar "undef"
